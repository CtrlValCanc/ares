...
static inline void __attribute__((noreturn)) sys_exit(int status) {
  asm volatile("li a7, 93      \n" // Load syscall number for exit (93) into a7
               "mv a0, %[code] \n" // Move status code to a0
               "ecall          \n" // Transfer control to operating system
               :                   // No output operands
               : [code] "r"(status)
               : "a7", "a0");
  while (1)
    ; // Ensure the compiler knows this point is unreachable
}
...
char heap[32*1024*1024];
int heap_bump = 0;

void* malloc(int size) {
    void* alloc = heap + heap_bump;
    heap_bump += size;
    return alloc;
}
...
int snprintf(char* buf, int n, const char *fmt, ...) {
	va_list val;
    va_start(val, fmt);
    int rv = npf_vsnprintf(buf, n, fmt, val);
    va_end(val);
    return rv;
}
void stderr() { panic("stderr"); }

void free() {  }
void fprintf() { panic("fprintf"); }
void __eqdf2() { panic("__eqdf2"); }
void puts(const char* c) { sys_write(1, c, strlen(c)); }
void putchar(char c) { sys_write(1, &c, 1); }
void printf_putchar(int ch, void* ctx) {
    putchar(ch);
}
char buf[1024];
int printf(const char *fmt, ...) {
    va_list val;
    va_start(val, fmt);
    int rv = npf_vsnprintf(buf, 1024, fmt, val);
    va_end(val);
    puts(buf);
    return rv;
} 
void exit() { panic("exit"); }
void strstr() { panic("strstr"); }
int abs(int x) { if (x >= 0) return x; return -x; }
void* fopen(char* str) { printf("fopen file %s\n", str); if (strcmp(str, "doom1.wad") != 0) return NULL; g_file_pos = 0; return (void*)1;  }
void fclose() { return; }
int ftell() { return g_file_pos; }
void remove() { panic("remove"); }
void rename() { panic("rename"); }
void __ctype_toupper_loc() { panic("__ctype_toupper_loc"); }
void stdout() { panic("stdout"); }
void fflush() { panic("fflush"); }
int vfprintf(void* file, char* fmt, va_list val) { 
    int rv = npf_vsnprintf(buf, 1024, fmt, val);
    puts(buf);
    return rv;
}
void system() { panic("system"); }
void atof() { panic("atof"); }
void __isoc99_sscanf() { panic("__isoc99_sscanf"); }
void mkdir() {  }
int real_errno;
int* __errno_location() { return &real_errno; }
#define SEEK_SET	0	
#define SEEK_CUR	1
#define SEEK_END	2	
int fseek(void* stream, long offset, int origin) {
	if (origin == SEEK_SET) g_file_pos = offset;
	if (origin == SEEK_CUR) g_file_pos += offset;
	if (origin == SEEK_END) g_file_pos = sizeof(doom1_wad) + offset;
	return 0;
}
size_t fwrite( void*  buffer, size_t size, size_t count,
               void*  stream ) {
	int s = size * count;
	memcpy(doom1_wad + g_file_pos, buffer, s);
	g_file_pos += s;
	return count;
}
size_t fread( void*  buffer, size_t size, size_t count,
               void*  stream ) {
	int s = size * count;
	memcpy(buffer, doom1_wad + g_file_pos, s);
	g_file_pos += s;
	return count;
}
int vsnprintf(char * NPF_RESTRICT buffer,
                                 size_t bufsz,
                                 char const * format,
                                 va_list vlist) { return npf_vsnprintf(buffer, bufsz, format, vlist); }
void* realloc(void* a, int size) { return malloc(size); }
void* calloc(int a, int b) { void* m = malloc(a*b); memset(m, 0, a*b); return m; }
void sscanf() { panic("sscanf"); }
void fabs() { panic("fabs"); }
void __floatsisf() { panic("__floatsisf"); }
void __mulsf3() { panic("__mulsf3"); }
void __divsf3() { panic("__divsf3"); }
void __extendsfdf2() { panic("__extendsfdf2"); }
void __truncdfsf2() { panic("__truncdfsf2"); }
void __addsf3() { panic("__addsf3"); }
void __ltdf2() { panic("__ltdf2"); }
void __fixsfsi() { panic("__fixsfsi"); }
void __trunctfdf2() { panic("__trunctfdf2"); }
